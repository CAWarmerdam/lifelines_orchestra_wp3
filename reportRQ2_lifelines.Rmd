...
isolation_variables <- c("isolation_adu_q_2_c", "isolation_adu_q_2_a", "isolation_adu_q_2_b")

# changing factor level order of isolation variables
orclu.l[, isolation_variables] <- lapply(orclu.l[,isolation_variables], factor, ordered = TRUE,
              levels=c(1,2,3), labels=c("rarely or never", "sometimes", "often"))

mini_variables <- c("minia3a_adu_q_2", "minia3f_adu_q_2", "minia1_adu_q_2", "minia3b_adu_q_2", "minio3a_adu_q_2", "minio1a_adu_q_2", "minio3e_adu_q_2")

# changing factor level order of isolation variables
orclu.l[, mini_variables] <- lapply(orclu.l[,mini_variables], factor, ordered = TRUE,
...
tabSleep.l <- tibble("Characteristics" = varCol.l , "Visit 2" = sleep.tab.l[,1])
...
tabDescr.CESD <- tibble("Question" = varCol.CESD , "Visit 2" = CESD.tab.l[,1])
...
# create descriptive table
UCLA.tab.l <- descrTabPub(varVec = varUCLA, nameVec = namesUCLA, database = orclu.l, print = FALSE, addN = TRUE)
...
tabDescr.UCLA <- tibble("Question" = varCol.UCLA , "Visit 2" = UCLA.tab.l[,1])

# adding extra line in table for question 1.1
tabDescr.UCLA <- rbind(c("In the last 14 days how often do you feel", NA), tabDescr.UCLA)
...
write.table(res.psqi, file = "Results/RQ2/Poisson/PSQI.csv", sep = ";", row.names = TRUE)
...
write.table(res.psqi2, file = "Results/RQ2/Poisson/PSQI2.csv", sep = ";", row.names = TRUE)
```
