...
# V2
tabDescrBase[which(tabDescrBase$Characteristics == "Hours of physical activity per week"),1] <- paste0(round(median(orclu.l$physical.activity[orclu.l$VISIT == "2"], na.rm = TRUE),2), " (", quantile(orclu.l$physical.activity[orclu.l$VISIT == "2"], na.rm = TRUE)[[2]], "; ", quantile(orclu.l$physical.activity[orclu.l$VISIT == "2"], na.rm = TRUE)[[1]], "); ", "N=",table(is.na(orclu.l$physical.activity[orclu.l$VISIT == "2"]))[[1]])


# table
# ft.base <- tabDescrBase %>% 
#     flextable() %>%
#     add_header_row(values = c("","Mean (SD)/ N (%)"),
#   colwidths = c(1,1)) %>%
